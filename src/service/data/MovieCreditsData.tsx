...
class MovieCreditsDataClass {
  ...
  public getMovieCast(id: string) {
    return MovieApi.getMovieCredits(id).then((credits) =>
      credits.cast.map(this.transformCastList)
    );
  }

  public getTVCast(id: string) {
    return MovieApi.getTVCredits(id).then((credits) =>
      credits.cast.map(this.transformCastList)
    );
  }
}
