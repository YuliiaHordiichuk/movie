...
class MovieApiClass {
  ...
  public async getMovieList(filter: string, config?: RequestConfig) {
    ...
    const data = await API.request<ListAPIResponse<MovieItemApi>>(
      endpoint,
      config
    );
    return data.results;
  ...
  public async getTvList(filter: string, config?: RequestConfig) {
    ...
    const data = await API.request<ListAPIResponse<TVItemApi>>(
      endpoint,
      config
    );
    return data.results;
  ...
  public async getTrendingTVList(time_window: string, config?: RequestConfig) {
